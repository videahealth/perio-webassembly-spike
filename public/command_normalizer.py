...
if __name__ == "__main__":
    test_cases = [
        ...
        "324",
        "3 2 4",
        "three three three four two three",  # Multi-triplet: ["333", "423"]
        "333423",  # Multi-triplet: ["333", "423"]
        "three three three four two",  # Incomplete: only ["333"]
        "bleeding on 11",
        "bleeding on eleven facial",
        "bleeding on all",
        "bleeding on quadrant 2",
        "jump to 15",
        "jump 20 lingual",
        "furcation on 14 grade 2",
        "plaque on all severe",
        "mobility on 8 grade 2",
        "undo",
        "repeat",
        "start",
        "save chart",
        ...
        "just random text",  # Should fail
    ]

    print("Testing command normalizer (process_transcript_all):\n")
    for test in test_cases:
        results = process_transcript_all(test)
        print(f"Input: '{test}'")
        for r in results:
            print(f"  -> {r}")
        print()
